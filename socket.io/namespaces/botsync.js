exports = module.exports = function(app, botSync, next) {
  ...
  var token = auth(botsyncConfig, request);
    ...
    socket.on("getUpdate", function() {
      tools.HTTPRequest(botsyncConfig.main.botConnect.address, botsyncConfig.main.botConnect.port, "/api/v1/bot/instances", "GET", {"Content-Type": "application/json", "Authorization": "bearer " + token}, "", function(data) {
        try {
          ...
          token = auth(botsyncConfig, request); // Re-authenticaiton
  ...
  async function auth(botsyncConfig, request) {
    var options = {
      method: "POST",
      uri: "http://" + botsyncConfig.main.botConnect.address + ":" + botsyncConfig.main.botConnect.port + "/api/v1/bot/login",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify(botsyncConfig.main.botLogin)
    }
